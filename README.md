...
**工具注册表契约**（v0.2.0 关键补丁）：
所有 25 个工具类实例必须保留 `.execute()` 方法 —— 注册器对 `Tool` 实例**只 mutate 字段（runtime），禁止 spread/replace**（否则丢失 prototype 方法）。回归测试见 `tests/tool-registry.test.mjs`，覆盖所有 25 个工具的 `execute` / `definition` / `runtime` 三件套。
...
Ovogo 已经过真实 LLM 端到端验证 —— **不是 mock、不是模拟、不是脚本化演示**。

**靶机**：`zhhovo.top`（用户自有授权靶场，`.ovogo/settings.json` 显式声明为唯一授权目标）
**驱动模型**：DeepSeek Chat（`deepseek-chat`，OpenAI 兼容协议）
**会话产物**：`sessions/zhhovo.top_2026-07-01_0837/`（~30 个扫描输出文件 + 4 个结构化 Finding）

#### 端到端跑通的能力（真实探测结果）

| 阶段 | 实际产物 / 发现 |
|------|---------------|
| **侦察** | 子 Agent 并行调度（recon / dns-recon / port-scan / web-probe / osint）真实执行 nmap / subfinder / amass / katana / httpx / ffuf / wafw00f |
| **服务识别** | nginx（HTTP 80）+ Tengine（HTTPS 443）+ OpenSSH 8.0；阿里云北京 39.106.227.104；3 个子域名（www / xinsheng / @）|
| **WAF 感知** | **长亭 SafeLine 雷池** —— 由 `sl-session` cookie 指纹识别，EnvAnalyzer 命中特征 |
| **CMS 探测** | WordPress 残留 + Elementor 3.32.3 / WP File Manager 8.0.2 / WP Fastest Cache 1.4.0 |
| **漏洞扫描** | nuclei 全模板 + nmap `--script vuln`：21+ nuclei findings、vulners 命中 **CVE-2023-38408**（OpenSSH 8.0 RCE，CVSS 9.8，8 条公开 exploit）|
| **认证攻击** | hydra SSH/WordPress 后台爆破任务（PID 已记录）|
| **结构化输出** | 自动写入 `recon_summary.txt` / `vuln_scan_summary.txt` / `.ovogo/findings/f00{1..4}.json` |

#### 诚实承认的限制

- **未拿到 flag**：SafeLine WAF 在前 + 弱密码爆破需时长 + WordPress 已下线 —— **这是真实环境的真实阻力，不是系统缺陷**
- **失败不代表无效**：同一份工具链在干净 Flask 靶机上 10/10 e2e 测试全绿（见下方"干净靶机复现"），证明系统本身能打，是真实靶机有防守
- **本节不是营销**：如果你想复现，跑 `scripts/demo-kill-chain.sh` 或 `tests/e2e/kill-chain.test.mjs`（10 个测试，覆盖侦察 → 漏洞探测 → PayloadGenerator → 利用 → flag 提取完整链路）

#### 修复记录（2026-07-01）

本次真实运行中发现并修复了一个**所有单测都没暴露**的关键 bug：

- **症状**：所有 9 个子 Agent 派发后第一轮工具调用全部抛 `tool.execute is not a function`
- **根因**：`src/tools/index.ts` 的 `withDefaultRuntime()` 用 `{...tool}` 浅展开类实例，把 prototype 上的 `execute` 方法丢了
- **修复**：改为 mutate `tool.runtime` 后返回原实例
- **回归测试**：`tests/tool-registry.test.mjs`（3 用例，遍历所有 25 个工具验证 `.execute` 仍是 function）
- **教训**：单测覆盖率 100% 也不能保证集成路径正确 —— 必须有**真实 LLM × 真实子 Agent × 真实工具调用**的端到端验证

### 干净靶机复现（无防护 Flask mock）

如果想看系统能打的版本，本仓库自带一个故意写坏的 Flask 靶机：

```bash
# 方式 1：脚本演示（5 步 kill chain，输出到终端）
./scripts/demo-kill-chain.sh

# 方式 2：单元化的 e2e 测试（10 个用例，CI 可跑）
npm run test:e2e

# 方式 3：真实 LLM 驱动（需要 DeepSeek / OpenAI key）
DEEPSEEK_API_KEY=sk-xxx node --test tests/e2e/agent-driven.test.mjs
```

靶机漏洞：SQLi on `/api/users?id=`、cmdi on `/api/ping?host=`、LFI on `/api/file?name=`、XSS on `/search?q=`。Mock flag 存储在 SQLite users 表中。
...
├── tests/
│   ├── *.test.mjs            # 单元测试 (163+ 用例)
│   └── e2e/
│       ├── kill-chain.test.mjs      # 10 用例：脚本化 kill chain 端到端
│       ├── agent-driven.test.mjs    # 真实 LLM 驱动端到端（需 API key）
│       ├── spawn-target.mjs         # 干净 Flask 靶机启动器
│       └── target/app.py            # 故意写坏的 Flask 靶机
├── scripts/
│   └── demo-kill-chain.sh           # 手动演示脚本（5 步 kill chain）
...
Ovogo 通过 `.ovogo/settings.json` 的 `engagement` 字段显式声明授权范围，**主 Agent 在每轮推理时都会读取并严格遵守**：
...
```json
{
  "engagement": {
    ...
    "end_date": "2026-08-01",
    "notes": "授权备注：例如'自有靶场'、'客户授权'等"
  }
}
```

- 主 Agent 只对 `targets[]` 内的目标发起主动探测
- 遇到 `out_of_scope[]` 中的目标会立即停下询问用户
- 系统 prompt（`src/prompts/system.ts`）会把以上约束注入每一轮 LLM 调用

> ⚠️ **演示目标说明**：本 README 截图和示例中出现的 `zhhovo.top` 是作者自有授权靶场（已与项目 `engagement.targets` 绑定），不在你的授权范围内 —— 请替换为你自己的目标。Ovogo **不会**在没有显式 `engagement.targets` 配置的情况下建议任何默认目标。
